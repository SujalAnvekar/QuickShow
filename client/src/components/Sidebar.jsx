...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  const { chats, setSelectedChat, theme, setTheme, user, navigate,createNewChat,axios,
    setChats,fetchUserChats,setToken,token} = useAppContext()
...
try {
  ...
  return (
    ...
    backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      className='flex justify-center items-center w-full py-2 mt-10 text-white bg-linear-to-r
...
rounded-md cursor-pointer group'>
        ...
        {user && <img 
        onClick={logout}
        src={assets.logout_icon} className='h-5 cursor-pointer hidden
